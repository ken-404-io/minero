                  ...
                  <tbody>
                    {users.map((u, i) => {
                      ...
                      return (
                        <tr key={u.id}>
                          <td>
                            <div className="flex items-center justify-center font-mono font-bold text-base" style={{ color: m ? undefined : "var(--text-muted)" }}>
                              {m ?? `#${rank}`}
                            </div>
                          </td>
                          <td>
                ...
                return (
                  <article key={u.id} className="card">
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        className="font-mono font-bold text-lg w-10 text-center flex-shrink-0"
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="font-mono font-bold" style={{ color: "var(--brand)" }}>
                          ₱{u.balance.toFixed(2)}
                        </div>
      ...
      {visiblePages.map((p, idx) => (
        <>
          {idx > 0 && visiblePages[idx - 1] !== p - 1 && (
            <span key={`ellipsis-${p}`} className="px-1 text-sm" style={{ color: "var(--text-muted)" }}>…</span>
          )}
          <Link
            key={p}
            href={`/admin/leaderboard?page=${p}`}
